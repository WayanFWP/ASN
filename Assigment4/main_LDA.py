...
y_inf = LDA.predict(X_inf_features)
...
lda_coef = np.abs(LDA.coef_[0])
top_features_idx = np.argsort(lda_coef)[-5:][::-1]

print("\n" + "="*50)
print("TOP 5 MOST IMPORTANT FEATURES:")
print("="*50)
for idx in top_features_idx:
    print(f"{feature_names[idx]}: {lda_coef[idx]:.4f}")
    
w = LDA.coef_[0]
b = LDA.intercept_[0]

print("\n" + "="*50)
print("LDA-NATIVE INFERENCE EXPLANATION")
print("="*50)
...
mean_confidence_lh = np.mean([LDA.predict_proba(X_inf_features[i:i+1])[0][0] * 100 
                               for i in range(len(y_inf)) if y_inf[i] == 0]) if np.sum(y_inf == 0) > 0 else 0
mean_confidence_rh = np.mean([LDA.predict_proba(X_inf_features[i:i+1])[0][1] * 100 
...
all_probs = LDA.predict_proba(X_inf_features)
